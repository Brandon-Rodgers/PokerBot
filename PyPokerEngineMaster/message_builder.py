...
class MessageBuilder:
  ...
  @classmethod
  def build_ask_message(self, player_pos, state, player_uuid):
    players = state["table"].seats.players
    player = players[player_pos]
    hole_card = DataEncoder.encode_player(player, holecard=True)["hole_card"]
    round_state = DataEncoder.encode_round_state(state)
    side_pot_totals = 0
    for i in range(len(round_state['pot']['side'])):
      side_pot_totals += round_state['pot']['side'][i-1]['amount']
    pot = round_state['pot']['main']['amount'] + side_pot_totals
    valid_actions = ActionChecker.legal_actions(players, player_pos, state["small_blind_amount"], pot)
    message = {
